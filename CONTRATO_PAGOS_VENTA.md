...
- [x] El acceso rápido desde Facturas y desde Albaranes abre el drawer correctamente preseleccionado. Verificado con datos reales, incluido un bug real encontrado y corregido en el camino: un cliente inactivo no aparecía en el `<Select>` aunque la preselección funcionara por dentro (fix: `clientesParaDrawer()` en `saldosVenta.js`, añade el cliente preseleccionado a la lista aunque esté inactivo). Una segunda regresión (la auto-aplicación podía marcar un documento distinto al preseleccionado si empataba en saldo) se encontró después y **ya está corregida y reverificada** — ver "Bug conocido..." al final de este documento.
...
## 9. Bug encontrado y corregido: la auto-aplicación podía desmarcar el documento preseleccionado (encontrado y corregido 2026-09-04)

Encontrado por el usuario probando el icono "Registrar cobro" en `FacturasVenta.jsx` (ya con el rediseño de `CONTRATO_UX_FACTURAS_VENTA.md`), pero el bug vive enteramente en `RegistrarPagoForm.jsx`, construido en este contrato — no es una regresión del contrato de UX de Facturas.

**Caso reproducido con datos reales**: cliente "Angel oder Äff", pulsar "Registrar cobro" en la factura `RE-2026-003` (saldo 50.00 CHF, fecha 11/05/2026). El drawer se abre con `documentoPreseleccionado` apuntando a esa factura y el monto precargado a 50.00 — pero el checkbox que queda marcado es el del albarán suelto `DN-260047` (saldo también 50.00 CHF, fecha 08/05/2026, anterior), no el de la factura.

**Causa**: el `useEffect` de auto-aplicación (líneas ~65-90 de `RegistrarPagoForm.jsx`) busca, cada vez que cambia `monto` o `documentos`, el primer documento de **toda la lista del cliente** (ordenada por fecha ascendente) cuyo saldo coincida exacto con el monto recibido — `documentos.find((d) => Math.abs(d.saldo - montoNum) < EPSILON)`. No tiene ninguna noción de `documentoPreseleccionado`; si otro documento del mismo cliente empata en saldo y tiene fecha anterior, ese gana el `.find()` y se marca en su lugar. Con datos reales existe el empate exacto (`DN-260047` y `RE-2026-003`, ambos a 50.00 CHF), así que el bug no es hipotético.

**Consecuencia real si no se detecta**: al guardar, el pago se aplicaría a `DN-260047` en vez de a `RE-2026-003` — la factura que el usuario quería cobrar seguiría "Pendiente" y un albarán ya cubierto por otra vía quedaría sobre-aplicado o duplicado, sin ningún aviso.

**Diagnosticado 2026-09-04, corregido el mismo día.** Fix en `RegistrarPagoForm.jsx`: el `useEffect` de auto-aplicación ahora distingue los dos casos desde el principio — si existe `documentoPreseleccionado`, ese documento se marca siempre directamente (buscado en `documentos` por `tipo`+`id`, con `monto: Math.min(doc.saldo, montoNum)` para no aplicar de más si el usuario cambia el monto recibido después), sin pasar nunca por la búsqueda genérica de coincidencia exacta contra el resto de la lista. Sin preselección (alta libre desde `Pagos.jsx`), el comportamiento original (exacta o FIFO) no cambia.

**Verificado con el caso real exacto que causó el bug**: mismo cliente "Angel oder Äff", mismo icono "Registrar cobro" en `RE-2026-003` desde `FacturasVenta.jsx` — con la corrección, el drawer marca ahora **`RE-2026-003 · 12/05/2026 · saldo 50.00 CHF`** (captura real), no `DN-260047`, a pesar de que ambos siguen empatando en 50 CHF (el empate no se resolvió ni se eliminó, se dejó de usar como criterio cuando hay preselección).

**Verificado también el camino sin preselección, para confirmar que no se rompió**: botón "Registrar pago" general de `Pagos.jsx`, cliente "Feld zu Tisch" (activo, con `RE-2026-010` pendiente por 120 CHF — "Angel oder Äff" es inactivo y no aparece en este `<Select>` sin preselección, así que se usó un cliente distinto para este camino), monto tecleado a mano 120 CHF → se marca `RE-2026-010` automáticamente por coincidencia exacta, igual que antes del fix.

Ningún dato real se escribió durante esta verificación (ambas pruebas se quedaron en observar qué queda marcado en el drawer, sin pulsar "Guardar pago").
