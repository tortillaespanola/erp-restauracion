...
- **Semielaborado-hijo**: reutiliza tal cual la fila YA calculada de la tabla de Semielaborados (`filasSemiPorId`, memo hoisted desde `filasSemiTodas`) -- misma necesidad agregada global y mismo badge de los 7 estados, cero consultas nuevas. Si el hijo no tiene fila propia (necesidad agregada 0 -- la cascada por déficit ya no le atribuye nada pendiente, addenda "necesidad neta en cascada"), se muestra como `ok` con su stock actual.
