...
## Addenda: traslado provisional del patrón de Semielaborados a Producto final — Capa A (2026-09-11)

**Producto final (`ProduccionProductosFinales.jsx`) sigue sin tener contrato propio** -- esta addenda NO es ese contrato. Es un traslado mecánico de mejoras ya probadas en Semielaborados (adendas de 2026-08-14 a 2026-09-10) a la implementación completamente separada y duplicada de Producto final, sin abrir ninguna decisión de diseño nueva sobre las particularidades reales de esa pantalla (producción contra pedido de cliente, generación de albarán -- "Capa B", sigue **pendiente, sesión aparte con su propio contrato y mockup**, sin arrancar).

**Auditoría previa (Paso 0):** `ProduccionProductosFinales.jsx` no comparte ningún componente ni función con `Producciones.jsx` más allá de `ui.jsx` genérico -- propio `IngredienteConsumo`, propio `cargarIngredientesConLotes`, propias tablas (`receta_producto_final`, `consumo_produccion_pf`, `producciones_producto_final`). De la lista de mejoras de Semielaborados, ya estaban resueltas en la tabla "Productos finales" de `PedidosDelDia.jsx` (comparten `construirFilaDesglose`/`RANGO_ESTADO` con la tabla de Semielaborados, sin tocar en esta addenda): reordenamiento por estado, desglose expandible con contribución por padre, y estado del desglose forzado a `ok` cuando el padre ya está OK. Sin tocar tampoco: botón play por fila en la tabla "Productos finales" (Capa B -- implica decidir cómo se abre una producción de cliente desde ahí, no es traslado mecánico).

**Particularidad real encontrada / diferencia deliberada preservada:** a diferencia de Semielaborados, Producto final tiene `pedido_id`/`tanda_id` como vínculo real a un pedido de cliente -- **no se toca, no se sustituye por `cantidad_objetivo`**. La cantidad sugerida por la tanda (`necesidadesDeTanda('producto_final')`) sigue siendo un cálculo en vivo, completamente aparte de la nueva `cantidad_objetivo` persistida (misma dualidad ya existente en Semielaborados entre el bloque "sugerida por la tanda" y el campo "Cantidad objetivo" -- aquí se traslada la mitad persistida/editable, no se inventa nada).

**Migración:** `supabase/migrations/20260911_cantidad_objetivo_produccion_producto_final.sql` -- `alter table producciones_producto_final add column cantidad_objetivo numeric`, mismo patrón que `20260902_cantidad_objetivo_produccion_semielaborado.sql`.

**Traslado aplicado en `ProduccionProductosFinales.jsx`:**
- `cargarEstimacionPF(productoFinalId)` -- nueva función, traslado literal de `cargarCadenaCompleta()` (Producciones.jsx) sobre `receta_producto_final` en vez de `receta_semielaborado`. Nivel directo de receta únicamente, mismo criterio que el resto de esta pantalla.
- `cargarIngredientesConLotes`: `articuloIdsDeReceta` hoisteado fuera del `if (esArticulo)` y expuesto en el objeto devuelto (mismo fix que ya tenía Semielaborados) -- necesario para cruzar consumo confirmado por articulo_id real, no por nombre.
- `selectCompleto` (en `cargarDatos()`): `entrada_material(articulo_id, ...)` y `producciones_semielaborado!...(semielaborado_id, ...)` -- exponen los ids reales que antes faltaban, mismo fix que Producciones.jsx.
- `ProduccionAbierta`: campo persistido "Cantidad objetivo" (siempre visible/editable, `guardarObjetivo()` al perder foco) reemplaza el hueco que había -- antes el campo de cantidad SOLO existía si `produccion.tanda_id`, así que una producción manual (sin tanda) no tenía forma de fijar cantidad objetivo en absoluto. Estimación/disponible fusionados en cada línea de "Registrar consumo" (`estimacionPorNombre`, `consumoRegistradoPorIngrediente`, `filasConEstimacion`), botón "Usar estimación" por línea (oculto si esa línea ya tiene consumo > 0 registrado -- mismo fix que la addenda de 2026-09-09), reordenamiento de líneas con consumo ≥ estimado al final, indicador informativo "Consumo suficiente para cerrar", y bloque de botones (Confirmar consumo / Cerrar producción) reposicionado arriba, antes de "Consumo ya registrado" -- visible sin scroll.
- **Decisión explícita distinta a la primera propuesta:** se elimina por completo el bloque "Cantidad a producir — sugerida por la tanda" y su botón "Recalcular consumos sugeridos" (mecanismo preexistente que rellenaba TODAS las líneas de golpe sin mirar consumo ya confirmado) -- sustituido únicamente por "Usar estimación" por línea. La preselección automática del lote de Mezcla recién cerrada bajo la misma tanda (función real, sin equivalente en Semielaborados) se conserva, pero separada en su propio efecto, ya no depende de la cantidad sugerida ni del botón eliminado.
- Formulario "Iniciar nueva producción": nuevo campo "Cantidad a producir (opcional)" (traslado de `cantidadPlan`, sin la validación previa de stock de Semielaborados -- no pedida), persistido como `cantidad_objetivo` al insertar.
- `cerrarProduccion()`: navega a `/pedidos-del-dia` en vez de solo llamar a `onCambio()` -- mismo criterio que la addenda "navegación tras cerrar producción de semielaborado (2026-09-05)".
- `IngredienteConsumo`: gana los mismos props opcionales `estimacion`/`cargandoEstimacion`/`onPrecargar` que Semielaborados (`ProduccionCerradaEdicion`, que también usa este componente, sigue llamándolo sin ellos, sin cambios). El panel de sustitución excepcional y el aviso de "fecha posterior" de lote de semielaborado (funciones propias de Producto final, sin equivalente en Semielaborados) se conservan intactos.

**Fuera de alcance de esta addenda** (sin tocar): botón play/inicio de producción desde la tabla "Productos finales" de Producciones del día; cualquier lógica de cliente, albarán o `pedidos_venta`; `necesidades_pedidos_cascada()`; `Producciones.jsx`/Semielaborados (referencia, no se modifica).
