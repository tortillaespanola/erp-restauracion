...
function Pedidos() {
  ...
  //
  // CONTRATO_HARDENING_A1_A4.md (A2): un pedido 'servido' es siempre el resultado del trigger
  // actualizar_estado_pedido_por_servicio(), que solo pone ese estado cuando TODAS las líneas están
  // cubiertas por lineas_albaran_venta reales -- para datos generados por el flujo normal, este
  // cálculo por línea y el estado de cabecera nunca pueden discrepar. La única fuente de discrepancia
  // encontrada es el histórico del negocio demo (scripts/demo-catering/poblar.mjs), que fuerza
  // estado='servido' a mano sin generar lineas_albaran_venta (decisión deliberada y documentada ahí,
  // para que "Pedidos del día" arranque limpio) -- eso hacía que esta función recalculase "0/N
  // servidas" junto a una badge "Servido", una contradicción visual sin ningún dato realmente
  ...
  // (el detalle expandido de abajo sigue mostrando el "Servido" real por línea sin cambios, para no
  // ocultar histórico incompleto). 'cancelado' no entra en este atajo: un pedido cancelado a medio
  // servir sí puede tener menos líneas servidas que el total, y eso es información real, no una
  // contradicción.
