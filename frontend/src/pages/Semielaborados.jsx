...
function Semielaborados() {
  ...
  async function cargarDatos() {
    ...
    const [resSemi, resArticulos] = await Promise.all([
      supabase
        .from('semielaborados')
        .select(`
          *,
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold">Semielaborados</h1>
      ...
      <form onSubmit={handleSubmit} className="mt-6 bg-white p-4 rounded-lg shadow flex flex-col gap-4">
        ...
        <div className="flex gap-2">
          <button type="submit" className="bg-slate-900 text-white rounded px-4 py-2 hover:bg-slate-700">
            {editandoId ? 'Guardar cambios' : 'Guardar semielaborado'}
          </button>
          {editandoId && (
      ...
      <div className="mt-8">
        ...
        {cargando ? (
          ...
          <p className="text-slate-500">Todavía no hay semielaborados dados de alta.</p>
        ) : (
              ...
              <div key={s.id} className="bg-white rounded-lg shadow p-4">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => handleEditar(s)} className="text-blue-600 hover:underline text-sm">
                      Editar
                    </button>
                    <button onClick={() => handleBorrar(s.id)} className="text-red-600 hover:underline text-sm">
                      Borrar
                    </button>
                  </div>
                </div>
