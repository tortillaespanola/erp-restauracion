...
✅ **Implementado — pantalla nueva `CierreTanda.jsx` (`?tanda_id=`), no integrada en `AlbaranesVenta.jsx`.** Decisión tomada al comparar: `AlbaranesVenta.jsx` es un editor línea a línea para UN pedido (elegir lote a mano por línea) — el problema de esta pantalla es distinto, calcular un reparto entre N pedidos a la vez a partir de una producción ya conocida, con una decisión de todo-o-nada antes de escribir. Forzarlo en el formulario existente habría significado rehacer su interacción; un modal se quedaba corto para la tabla de pedidos + déficit editable + líneas heredadas. Sin entrada propia en el menú lateral — solo alcanzable vía el enlace "Cerrar y repartir →" en Pedidos del día, visible cuando el grupo ya tiene "Producto final generado".

Una tanda puede tener pedidos de más de un producto final (Parte 1, tanda compartible) — el déficit se calcula **por producto**, no globalizado para toda la tanda; cada sección de producto tiene su propia tabla de pedidos y, si hay déficit, sus propios inputs editables (precargados con un reparto FIFO por orden de llegada como sugerencia de partida, nunca escrito hasta confirmar). El reparto entre lotes también es FIFO si hay más de una producción cerrada del mismo producto en la tanda.

Probado en runtime real, login real, en las dos pasadas planificadas, cada una encadenada desde cero con las Partes 1-3 reales:

**Pasada 1 — sin déficit:** 1 pedido (Zum Kuss, 1 ud.), producción cerrada con `cantidad_producida = 1` → badge "Sin déficit — reparto automático", tabla no editable. Al confirmar: 1 albarán, 1 línea (`produccion_pf_id` correcto, `precio_unitario` heredado del pedido, `linea_pedido_id` vinculado), pedido pasó a `servido`, **0 incidencias nuevas** (la única fila de `incidencias_stock_producto_final` en el sistema es la real preexistente, verificado explícitamente que no creció).

**Pasada 2 — con déficit + herencia de línea libre:** 2 pedidos del mismo producto y fecha (Zum Kuss 2 ud. + una línea "Pan" de texto libre; Feld zu Tisch 1 ud.), producción cerrada deliberadamente con `cantidad_producida = 2` (déficit de 1 sobre un total pedido de 3) → badge "Déficit: solo hay 2.000 de 3.000", reparto FIFO por defecto (Zum Kuss 2 / Feld zu Tisch 0), y la sección "Líneas de texto libre que se heredan al reparto" mostrando correctamente el "Pan" de Zum Kuss antes de confirmar nada. Reparto reajustado a mano a 1/1 y confirmado: 2 albaranes — el de Zum Kuss con la línea de Tortilla (1 ud.) **más la línea heredada "Pan"** (2 ud., precio correcto); el de Feld zu Tisch solo con su línea de Tortilla. Exactamente **1 incidencia** insertada (Zum Kuss, `cantidad_negativa = 1`, `motivo = 'stock_negativo'`, `linea_albaran_venta_id` apuntando a la línea real recién creada) — Feld zu Tisch no genera incidencia porque recibió el 100% de lo que le tocaba (1 de 1), confirmando que el cálculo es por pedido, no por producto. Zum Kuss quedó en estado `pendiente` (entrega parcial), Feld zu Tisch en `servido`.

0 errores de consola y 0 diálogos de alerta en ambas pasadas. Las producciones y albaranes reales preexistentes quedaron intactos — verificado explícitamente antes y después de cada pasada. Todo lo de prueba (albaranes, líneas, incidencia, producciones, pedidos, tanda) borrado al terminar; el stock de los lotes reales usados volvió exactamente a sus valores anteriores en ambos casos.

**Simplificación consciente, no cubierta todavía**: un pedido que en el reparto recibe exactamente 0 (déficit total, no parcial) no genera ni albarán ni incidencia en esta versión — queda pendiente sin más rastro, a la espera de una tanda futura. Cubrir ese caso (¿incidencia igualmente, sin `linea_albaran_venta_id`?) queda para cuando haya un caso real, mismo criterio que el resto de este documento.
