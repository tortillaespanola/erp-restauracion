...
function validarEntrada(input) {
  const { nombre_negocio, codigo_corto, email_admin } = input || {}
  ...
  return { nombre_negocio: nombre_negocio.trim(), codigo_corto, email_admin: email_admin.trim() }
}
...
export async function crearEmpresa(deps, callerUserId, input) {
  ...
  const { nombre_negocio, codigo_corto, email_admin } = validarEntrada(input)
  ...
  try {
    ...
    const usuario = await deps.invitarUsuario(email_admin)
